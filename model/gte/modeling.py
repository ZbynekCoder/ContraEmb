...
def cl_forward(
        ...
        # Query-side transform ONLY on z1 (query/anchor)
        if getattr(self.model_args, "use_query_transform", False):
            ...
            if hasattr(self, "query_transform") and self.query_transform is not None:
                z1 = z1 + scale * self.query_transform_dropout(self.query_transform(z1))
...
class NewModelForCL(NewPreTrainedModel):
    ...
    def __init__(self, config: NewConfig, model_args, add_pooling_layer=False):
        ...
        # ====== Query-side learnable transform T (only affects query/anchor z1) ======
        self.use_query_transform = getattr(self.model_args, "use_query_transform", False)
        if self.use_query_transform:
            self.query_transform_dropout = nn.Dropout(getattr(self.model_args, "query_transform_dropout", 0.1))
            self.query_transform = nn.Linear(config.hidden_size, config.hidden_size, bias=False)

            # Init for stability (match our_BertForCL behavior)
            init_std = float(getattr(self.model_args, "query_transform_init_std", 0.02))
            nn.init.normal_(self.query_transform.weight, mean=0.0, std=init_std)
        else:
