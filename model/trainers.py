                ...
                if step % (len(epoch_iterator) // 3) == 0 and step > 0:
                    ...
                    for key, value in actual_model.custom_epoch_info.items():
                        if value:  # Check if the list is not empty
                            # Move tensors to CPU and compute the average
                            avg_values = [v.cpu() for v in value if v.is_cuda]
                            if avg_values:  # Ensure the list is not empty
                                avg_custom_epoch_info[key] = round(sum(avg_values).item() / len(avg_values), 3)

                    # Reset the custom epoch info for the next calculation
                    actual_model.custom_epoch_info = {key: [] for key in actual_model.custom_epoch_info}

                    # Perform evaluation and calculate average of custom epoch information post-evaluation
                    eval_results = round(self.evaluate().item(), 3)  # Assuming this returns a scalar
                    eval_avg_custom_epoch_info = {}
                    for key, value in actual_model.custom_epoch_info.items():
                        if value:  # Check if the list is not empty
                            # Similarly, move tensors to CPU for the evaluation info
                            eval_values = [v.cpu() for v in value if v.is_cuda]
                            if eval_values:  # Ensure the list is not empty
                                eval_avg_custom_epoch_info[key] = round(sum(eval_values).item() / len(eval_values), 3)

                    # Reset the custom epoch info again after evaluation
                    actual_model.custom_epoch_info = {key: [] for key in actual_model.custom_epoch_info}
